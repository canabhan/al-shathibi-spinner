...
const Toss = () => {
  ...
  const id = useParams();
  const { subs, updateSub, allCandidates, isPopup, sorted, setIsPopup } =
    useContext(CandidateContextProvider);
  ...
  const checkUser = async () => {
    ...
    const user = await allCandidates.find((item) => {
      return (item.stCode || item.code) == id.id;
    });
  ...
  const handleSub = async (idd, subject) => {
    console.log("dfdadaa",subject,idd);
    const user = await allCandidates.find((item) => {
      return (item.stCode || item.code) == id.id;
    });
    ...
    background:
      ...
      {is ? (
        <>
          ...
          <div className="bg-[#] h-screen  !overflow-hidden relative flex flex-col justify-center">
            <div
            ...
            <img
              src={v3}
              className="fixed w-[700px] z-8 -left-[450px]  top-1/2 -translate-y-1/2 animate-[spin_35s_linear_infinite]"
              alt=""
            />
            <img
              src={v3}
              className="fixed w-[700px] z-8 -right-[450px]  top-1/2 -translate-y-1/2 animate-[spin_35s_linear_infinite]"
              alt=""
            />
            ...
            {/* <div className="flex gap-8 justify-center items-center mt-5 ">
              ...
              <div className="flex gap-6">
                ...
                <img
                  src={first}
                  className="w-25 h-25 object-cover scale-115"
                  alt=""
                />
            ...
            <div className="relative z-10 flex justify-center items-center mt-8">
              <div className="bg-[#fef6ea] flex items-center justify-center w-2/3 h-[430px] rounded-4xl shadow-[0_5px_15px_rgba(0,0,0,0.35)]">
                      ...
                      <>
                        {!item.locked ? (
                          <div data-aos="fade-up">
                            <div
                              className="bg-[#cc4104] w-30 h-30 text-center rounded-3xl flex items-center justify-center text-3xl shadow-4xl font-bold text-[#fef7eb] border-6 border-[#d6510d] hover:scale-105 transition-transform duration-300"
                              key={index}
                              onClick={() => handleSub(item.id, item.name)}
                            >
                              {index + 1}
                            </div>
                          </div>
                          ...
                          <div data-aos="fade-up">
                            <div className="bg-[#d6510d] w-30 h-30 text-center rounded-3xl flex items-center justify-center text-3xl shadow-4xl font-bold text-[#fef7eb] border-6 border-[#cc4104] hover:scale-105 transition-transform duration-300">
                              <FaLock />
                            </div>
                          </div>
                        )}
